...
class ImageEmbedding(nn.Module):
    def __init__(self, patch_size=(16, 16),
        ...
        fh, fw = patch_size
        # self.double_conv = DoubleConv(in_channels= in_channels, out_channels=conv_channels)
        # self.resblock = ResidualBlock(conv_channels)
        self.embedding = nn.Conv2d(in_channels, emb_dim, kernel_size=(fh, fw), stride=(fh, fw))
    def forward(self, x):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        num_patches = gh * gw
        self.embedding = nn.Conv2d(in_channels, emb_dim, kernel_size=(fh, fw), stride=(fh, fw))
        #self.embedding = ImageEmbedding(patch_size=patch_size, emb_dim=emb_dim, in_channels=in_channels)
        # class token
...
if __name__ == '__main__':
    ...
    model = VisionTransformer( image_size=(608,800), patch_size=(38,50), num_layers=2)
    x = torch.randn((2, 3, 608, 800))
    out = model(x)
